-- Harden checkout validation at the private order-creation boundary.
-- Road and house stay digit-only after trimming; phone accepts digits plus one
-- optional leading international-prefix sign. Preserve the typed event actor
-- cast, private signature, SECURITY DEFINER settings, and existing ACLs.
...
begin
  ...
  if p_full_name is null or char_length(btrim(p_full_name)) not between 2 and 120
     or p_road_number is null
     or char_length(btrim(p_road_number)) not between 1 and 40
     or btrim(p_road_number) !~ '^[0-9]+$'
     or p_house_number is null
     or char_length(btrim(p_house_number)) not between 1 and 40
     or btrim(p_house_number) !~ '^[0-9]+$'
     or p_flat_number is null
     or char_length(btrim(p_flat_number)) not between 1 and 40
     or p_phone is null
